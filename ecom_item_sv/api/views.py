...
class ItemsViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        #     serializer.save()
        #     return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response({"error": "Can not create item"}, status=status.HTTP_400_BAD_REQUEST)

    def update(self, request, pk=None):
        return Response({"error": "Can not update item"}, status=status.HTTP_404_NOT_FOUND)
        # try:
